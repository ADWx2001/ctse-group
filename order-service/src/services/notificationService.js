...
const sendOrderConfirmation = async (orderData) => {
  try {
    ...
    console.log(`[Order Service] Notification sent for order ${orderData.orderId}`);
  } catch (err) {
    // Notification failure is non-critical — log and continue
    console.error(`[Order Service] Failed to send notification: ${err.message}`);
  }
};
...
const sendStatusUpdate = async (orderData, newStatus) => {
  try {
    ...
    console.error(`[Order Service] Failed to send status notification: ${err.message}`);
  }
};
